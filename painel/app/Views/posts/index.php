<div class="col py-3">
    ...
    <div class="container">
        <div class="d-flex justify-content-between align-items-center mb-4">
            ...
            <div>
                <?php if (!$isTrash): ?>
                    <a href="<?= base_url('posts/trash') ?>" class="btn btn-warning me-2">
                ...
                <?php else: ?>
                    <a href="<?= base_url('posts') ?>" class="btn btn-secondary">
                        <i class="bi bi-arrow-left me-2"></i>Voltar aos Posts
                    </a>
                <?php endif; ?>
            </div>
        </div>
            ...
            <div class="card">
                <div class="card-header">
                    <h5 class="card-title mb-0">
                        <?= $isTrash ? 'Posts na Lixeira' : 'Lista de Posts' ?>
                    </h5>
                </div>
                            ...
                            <div class="d-flex justify-content-between align-items-center">
                                ...
                                <div>
                                    <small class="text-muted me-3">
                                        <?php echo '<i>Criado em </i>' . date('d/m/Y', strtotime($post['created_at'])) ?>
                                    </small>
                                    <?php if ($user['role'] === 'admin' || $post['user_id'] === $user['id']): ?>
                                        <?php if ($isTrash): ?>
                                            <a href="<?= base_url('posts/restore/' . $post['id']) ?>" 
                            ...
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        <?php if ($isTrash): ?>
                                            <p>Tem certeza que deseja excluir permanentemente este post?</p>
                                            <small class="text-danger">Esta ação não poderá ser desfeita!</small>
                                        <?php else: ?>
                                            <p>Tem certeza que deseja mover este post para a lixeira?</p>
                                        <?php endif; ?>
                                    </div>
                                    <div class="modal-footer">
                                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancelar</button>
                                        <?php if ($isTrash): ?>
                                            <a href="<?= base_url('posts/forceDelete/' . $post['id']) ?>" 
